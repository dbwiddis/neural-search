...
public abstract class HybridLeafCollector implements LeafCollector {
    ...
    void populateScoresFromHybridQueryScorer() throws IOException {
        ...
        // hybridScores only credits sub-queries that are verified matches on the current doc. Comparing each
        // sub-scorer's own docID() instead would use the approximation's position, which for a two-phase
        // sub-query is a candidate rather than a confirmed match.
        hybridQueryScorer.hybridScores(compoundQueryScorer.getSubQueryScores());
    }
